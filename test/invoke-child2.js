	...
	if (++emitCount == times) {
		...
		/*eventPromises.forEach((promise) => {
			Promise.resolve(promise).then((result) => {
				logger.info("event received at peer:", peerUrl, "\ntx_id:", result);
				return;
			}).catch((err) => {
				logger.warn("event not received in ", config.eventWaitTime, "ms at peer:", peerUrl, "\ntx_id:", err);
				return;
			});
		})*/
		Promise.all(eventPromises).then((results) => {
			...
			logger.warn("events not all received in ", config.eventWaitTime, "ms at peer:", peerUrl, "\nwhich tx_ids not received :", table);
			return;
		});
	}
});
...
var oneRoundInvoke = function(user, roundNum, invokeTimes, endRound) {
	...
	for (var i = 0; i < invokeTimes; i++) {
		...
		send(user, args).then(collectEndorsment, (err) => {
			...
			logger.info("return No. ", ++returnCount, "/", total,
				"invoke send to orderer after:", (endTime - startTime).toString(), "ms\n",
				"tx_id:", message);
		})
	}
...
var send = function(user, args) {
	...
	table[tx_id.getTransactionID().toString()] = ++sendCount;
